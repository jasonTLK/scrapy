...
class hz_gov_Spider(scrapy.Spider):
    ...
    def start_requests(self):
        urls = [
            "http://ggzyw.ningde.gov.cn:9120/tenderproject/GetTpAllInfo?name=&page="
        ]
        pages = [79]
        for i in range(len(urls)):
            num=1
            while num<= pages:
                url =urls[i]+str(num) + "&records=15&status=&publishBeginTime=&publishEndTime="
                num+=1
                # print url
