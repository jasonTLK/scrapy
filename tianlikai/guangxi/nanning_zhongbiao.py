...
class hz_gov_Spider(scrapy.Spider):
    name = "nanning_zhongbiao.py"
    allowed_domains = ["nnggzy.net"]
    custom_settings = {
    ...
    def start_requests(self):
        urls = [
            "http://www.nnggzy.net/gxnnzbw/showinfo/zbgsmore.aspx"
        ]
        pages = [337]
        for i in range(len(urls)):
            yield Request(url=urls[i], callback=self.parse, meta={'url': urls[i], 'page': pages[i]})
