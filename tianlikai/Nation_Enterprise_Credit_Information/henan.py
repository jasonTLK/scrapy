...
class hz_gov_Spider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        # m = selector.xpath("//div[@class='baseinfo']//p[@class='text mainPerText']/span/text()").extract()
        # type4 = selector.xpath("//div[@class='baseinfo']["+str(2)+"]//div[@class='ax_table liebiaoxinxin']//td/text()").extract()
        # for i in type4:
        #     print ''.join(i)
        ...
        # mm = [u'营业执照信息']
        # for i in m:
        #     if len(''.join(i).strip().replace(' ','')):
        #         mm.append(''.join(i).strip().replace(' ','').replace('&nbsp',''))
        # mm = list(set(mm))
