...
class hz_gov_Spider(scrapy.Spider):
    ...
    def start_requests(self):
        page =1
        while page<=201:
            url = "http://ggzyjy.dl.gov.cn/TPFront/jyxx/071002/071002003/?Paging="+str(page)
            page+=1
            # print url
    ...
    def parse(self, response):
        ...
        for i in range(len(names)):
            str ="".join(names[i]) + "," + "http://ggzyjy.dl.gov.cn" + "".join(urls[i])
    ...
    def parse2(self, response):
        ...
        items = biding_gov()
        items["url"] = response.url.decode("gbk")
